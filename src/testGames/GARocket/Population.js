...
export default class Population {
  constructor(/** @type {GA_Rocket} */ gameObj) {
    ...
    for (let i = 0; i < this.populationSize; i++) {
      this.rockets[i] = new Rocket(
        this.m_game.m_canvasWidth / 2,
        this.m_game.m_canvasHeight - 80,
        this.m_game
      );
    }
  }
  ...
  evaluate() {
    ...
    for (let i = 0; i < this.populationSize; i++) {
      ...
      if (this.rockets[i].fitness >= 0.8) n += 30;
      if (this.rockets[i].fitness >= 0.85) n += 40;
      if (this.rockets[i].fitness >= 0.9) n += 50;
  ...
  naturalSelection() {
    ...
    for (let i = 0; i < this.populationSize; i++) {
      /** @type {Individual_DNA} */
      parentA_DNA = utils.getRandomEl(
        this.selectionPool,
        this.selectionPoolSize
      ).individual_dna;
      parentB_DNA = utils.getRandomEl(
        this.selectionPool,
        this.selectionPoolSize
      ).individual_dna;
