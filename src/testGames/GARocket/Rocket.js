...
export default class Rocket {
  ...
  calFitness() {
    if (this.reachTheTarget) {
      ...
      if (this.disToTarget < this.m_game.m_canvasHeight) {
        ...
        this.fitness =
          1 -
          utils.scaleNumberInRange(
            this.disToTarget,
            0,
            this.m_game.m_canvasHeight,
            0,
            1
          );
      }
    ...
    if (this.crashed) {
      if (
        this.disToTarget <= 165 &&
        Math.abs(this.pos.y - this.m_game.target.pos.y) <= 100
      )
        this.fitness *= 9 / 10;
      else if (
        this.disToTarget <= 165 &&
        Math.abs(this.pos.y - this.m_game.target.pos.y) <= 80
      )
        this.fitness *= 95 / 10;
      else if (
        this.disToTarget <= 190 &&
        Math.abs(this.pos.y - this.m_game.target.pos.y) <= 100
      )
        this.fitness *= 9 / 10;
      else if (
  ...
  render() {
    ...
    this.m_game.FillRectAndRotate(
      this.pos.x,
      this.pos.y,
      this.width,
      this.height,
      this.rocketRotateAngle,
      this.color
    );
  }
}
